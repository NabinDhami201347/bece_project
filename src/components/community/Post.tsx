...
const Post: React.FC<PostProps> = ({ title, description, author, date, image }) => {
  const navigation = useNavigation();
  const [liked, setLiked] = useState(false);

  const onPress = () => {
    // @ts-ignore
    navigation.navigate("Post", { title, description, author, date, image });
  };
    ...
    <TouchableOpacity onPress={onPress} style={styles.container}>
      ...
      <View style={styles.reactionsContainer}>
        <TouchableOpacity onPress={handleLikePress} style={styles.reactionButton}>
          <Ionicons name={liked ? "thumbs-up" : "thumbs-up-outline"} size={20} color={liked ? "blue" : "gray"} />
          <Text style={styles.reactionText}>Like</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={handleCommentPress} style={styles.reactionButton}>
          <Ionicons name="chatbubble-outline" size={20} color="blue" />
          <Text style={styles.reactionText}>Comment</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={handleSharePress} style={styles.reactionButton}>
